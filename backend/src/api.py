...
def create_new_drink(token):
    try:
        ...
        recipe_ = request.get_json().get("recipe")
        lst = []
        if not isinstance(recipe_, list):
            lst.append(recipe_)
            recipe_ = lst
        print(type(recipe_), type(recipe_[0]))
        recipe_ = json.dumps(recipe_)
        drink = Drink(title=title, recipe=recipe_)
        drink.insert()
...
def update_drink_data(token, id):
    try:
        ...
        recipe_ = request.get_json().get("recipe")
        print(type(recipe_))
        if not isinstance(recipe_, list) and recipe_ is not None:
            recipe_ = list(recipe_)
        print(type(recipe_))
        drink = Drink.query.filter(Drink.id == id).first()
        if title is not None:
            drink.title = title
        if recipe_ is not None:
